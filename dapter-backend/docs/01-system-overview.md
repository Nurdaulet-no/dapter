...
Dapter API converts uploaded study files (PDF, PPTX, TXT, MD) into one of two artifact types:
...
The user picks which artifact to generate per upload. The two surfaces are completely parallel and independent.

## Key properties

- One upload → one row. A single `flashcards` row owns its full deck via JSON `content`; a single `quizzes` row owns its full quiz the same way. Children (cards, questions) are not separate collections.
...
2. Client `POST`s to `/flashcards/` **or** `/quizzes/` with 1–5 files.
...
7. Flashcards only: image-generation workers populate per-card `imageUrls` after the deck is persisted.
